...
import { getMixedRecommendations, clearMixedRecommendationCache } from '../utils/mixedRecommendation.js'
...
const testSorting = async () => {
  try {
    ...
    result.recommendations.forEach((rec, index) => {
      console.log(`${index + 1}. ID: ${rec._id}, 總分: ${rec.total_score}, 類型: ${rec.recommendation_type}`)
    })
    ...
    for (let i = 1; i < result.recommendations.length; i++) {
      if (result.recommendations[i].total_score > result.recommendations[i - 1].total_score) {
        ...
        console.log(`❌ 排序錯誤: 第${i+1}項分數(${result.recommendations[i].total_score}) > 第${i}項分數(${result.recommendations[i-1].total_score})`)
        break
      }
    }
    ...
    console.log(`- 最低分: ${result.recommendations[result.recommendations.length - 1]?.total_score || 'N/A'}`)
