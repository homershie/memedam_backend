...
vi.mock('../../../models/Meme.js', () => ({ default: { findById: vi.fn() } }))
...
describe('社交層分數計算系統', () => {
  ...
  describe.skip('SOCIAL_SCORE_CONFIG', () => {
    test('應該有正確的配置結構', () => {
  ...
  describe('getUserSocialInfluenceStats', () => {
    test('應該返回用戶社交影響力統計', async () => {
      const { getUserSocialInfluenceStats } = await import(
        '../../../utils/socialScoreCalculator.js'
      )
      ...
      const buildSpy = vi
        .spyOn(socialScoreModule, 'buildSocialGraph')
        .mockImplementation(mockBuildSocialGraph)
      const calcSpy = vi
        .spyOn(socialScoreModule, 'calculateSocialInfluenceScore')
        .mockImplementation(mockCalculateSocialInfluenceScore)
