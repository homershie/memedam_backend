...
├── utils/               # 工具函數
│   ├── advancedSearch.js        # 進階搜尋演算法
│   ├── collaborativeFiltering.js # 協同過濾演算法
│   ├── contentBased.js          # 內容基礎推薦
│   ├── hotScore.js              # 熱門分數演算法
│   ├── logger.js                # 日誌工具
...
- **config/** - 所有配置檔案，包括資料庫、快取、認證等
- **controllers/** - 業務邏輯控制器，處理請求和響應
- **middleware/** - Express 中間件，處理認證、限流、錯誤等
- **models/** - Mongoose 資料模型定義
- **routes/** - API 路由定義和端點映射
- **services/** ⭐ - 服務層，包含所有外部服務整合和排程器
- **utils/** - 純工具函數和演算法實現
- **test/** - 完整的測試套件
- **docs/** - 專案文檔和 API 說明
- **scripts/** - 維護和部署腳本
