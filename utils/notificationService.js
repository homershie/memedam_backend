...
export const createNewCommentNotification = async (memeId, commentUserId, commentContent) => {
  try {
    // 獲取迷因信息和作者
    const meme = await Meme.findById(memeId, 'author_id title').populate('author_id', 'username display_name')
 ...
 */
export const createMentionNotifications = async (content, mentionerUserId, memeId = null, contextType = 'comment') => {
  try {
    // 使用正則表達式找出所有提及的用戶名
...
export const createHotContentNotifications = async (hotMemes, targetUserIds = null) => {
  try {
    ...
    let users = []
    if (targetUserIds) {
      users = await User.find({ _id: { $in: targetUserIds } }, '_id username')
    } else {
      ...
      }, '_id username').limit(1000) // 限制數量避免過載
    }

    const memeTitle = hotMemes.length === 1 ? 
      (hotMemes[0].title || '熱門迷因') : 
      `${hotMemes.length} 個熱門迷因`
